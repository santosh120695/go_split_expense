...
import { Home, BarChart3, LogOut, Users, X } from "lucide-react";
import useAuthStore, { useSidebarStore } from "../store/useStore.ts";
...
const Sidebar = () => {
  ...
  return (
    <React.Fragment>
      <div
        className={`sidebar h-full bg-(--card) text-card-foreground border-r border-[#C5C3C3] flex flex-col fixed top-0 left-0 w-64 transform ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:relative md:translate-x-0 transition-transform duration-200 ease-in-out z-50`}
      >
        <div className="border-b border-[#C5C3C3] bg-(--primary) text-white p-4 flex items-center justify-between">
        ...
        {user && (
          <div className="p-4 text-center border-b border-[#C5C3C3] bg-(--card)">
            <p className="text-lg font-semibold text-foreground">Hello, {user.user_name}!</p>
          </div>
        )}
